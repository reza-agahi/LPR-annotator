...
$scope.getCurrentActiveBox = function(boxes) {
  for (var i = 0; i < boxes.length; i++) {
    if (boxes[i].x == $scope.canvas.getActiveObject().get('left') &&
        ...
        boxes[i].w == $scope.canvas.getActiveObject().get('width') &&
        boxes[i].h == $scope.canvas.getActiveObject().get('height')) {
          return boxes[i];
    }
  }
};
...
$scope.setDimensionsOfCurrentActiveBox = function(box) {
  ...
  box.w = $scope.canvas.getActiveObject().get('width');
  box.h = $scope.canvas.getActiveObject().get('height');
};
  ...
  $scope.indexOfActiveBox = function() {
    for (var i = 0; i < $scope.currentPlate.boxes.length; i++) {
      if($scope.currentPlate.boxes[i].x == $scope.canvas.getActiveObject().get('left') &&
          ...
          $scope.currentPlate.boxes[i].w == $scope.canvas.getActiveObject().get('width') &&
          $scope.currentPlate.boxes[i].h == $scope.canvas.getActiveObject().get('height'))
          return i;
    }
  }
